...
for epoch in range(num_epochs):
    ...
    for inputs, labels_type, labels_fresh in train_loader:
        ...
        labels = (labels_fresh).to(device).float()  # Multiclass labels (long for CrossEntropyLoss)       
    ...
    with torch.no_grad():  # Disable gradient computation during validation
        for inputs, labels_type, labels_fresh in val_loader:
            # Move data to the device
            inputs = inputs.to(device)
            labels = (labels_fresh).to(device).float()
...
torch.save(model.state_dict(), "alexnet_fresh.pth")
print("Model saved to alexnet_fresh.pth")